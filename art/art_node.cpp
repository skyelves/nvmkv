...
static art_node *art_node_grow(art_node *an) {
    ...
    switch (get_type(version)) {
        case node4: {
            art_node16 *an16 = (art_node16 *) (new_node = new_art_node16());
            art_node4 *an4 = (art_node4 *) an;
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (!is_leaf(an4->child[i]))
                    an4->child[i]->parent = new_node;
            }
            an16->version = set_count(an16->version, 4);
        }
            break;
        case node16: {
            art_node48 *an48 = (art_node48 *) (new_node = new_art_node48());
            art_node16 *an16 = (art_node16 *) an;
            ...
            break;
        case node48: {
            art_node256 *an256 = (art_node256 *) (new_node = new_art_node256());
            art_node48 *an48 = (art_node48 *) an;
            ...
            for (int i = 0; i < 256; ++i) {
                ...
                if (index) {
                    ...
                    if (!is_leaf(an48->child[index - 1]))
                        an48->child[index - 1]->parent = new_node;
                }
            }
        }
            break;
    ...
    art_node_set_offset(new_node, get_offset(version));
    art_node_set_new_node(an, new_node);
...
int art_node_is_full(art_node *an) {
    ...
    switch (get_type(version)) {
        case node4 :
            return get_count(version) == 4;
        case node16:
            return get_count(version) == 16;
        case node48:
            return get_count(version) == 48;
        default:
            return 0;
    }
}
...
art_node *art_node_replace_leaf_child(art_node *an, const void *key, size_t len, size_t off) {
    ...
    assert(i - off <= 8);
    art_node_set_prefix(new_node, k1, off, i - off);
    off = i;
    unsigned char byte;
...
void art_node_replace_child(art_node *parent, unsigned char byte, art_node *old, art_node *new_node) {
    ...
    __atomic_store(child, &new_node, __ATOMIC_RELEASE);
