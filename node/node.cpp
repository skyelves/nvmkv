...
void node_init(node *n, uint8_t type, uint8_t level) {
    ...
    n->pre = 0;
    n->sopt = 0;
    // multiple threads can be creating new_node node at the same time, increase node-id atomically
...
// if we can do a prefix compression and fit the new_node key in this node, return 1; else return 0
...
static int node_try_prefix_compression(node *n, const void *key, uint32_t len) {
    ...
    // see if we can get new_node prefix for all the key
    assert(n->keys);
    index_t *index = node_index(n);
    for (uint32_t i = 0; i < n->keys; ++i) {
    ...
    // now calculate whether we can fit the new_node key in this node after prefix compression
    uint32_t new_off = n->off - prelen * n->keys + key_byte + len + 1 + value_bytes + index_byte;
    if ((n->data + new_off) > (char *)index)
      return 0;

    // copy new_node node content to `buf`, adjust node index at the same time
    char buf[node_size];
    uint32_t off = 0;
    ...
    // copy new_node node content back
    memcpy(n->data, buf, off);
    // assign new_node offset
    n->off = off;
    // assign new_node prefix length
    n->pre += prelen;
    return 1;
#else
    (void) n;
...
int node_insert(node *n, const void *key, uint32_t len, const void *val) {
    ...
    if (unlikely((n->data + (n->off + key_byte + len1 + value_bytes + index_byte)) > (char *) index)) {
        ...
        // need to update new_node key suffix
        key1 = (char *) key + n->pre;
        len1 = len - n->pre;
...
// split half of the node entries from `old` to `new_node`
void node_split(node *old, node *new_node, char *pkey, uint32_t *plen) {
    ...
    *plen += flen;
    ...
    if (unlikely(old->level)) { // assign first child if it's not a level 0 node
        new_node->first = static_cast<node *>(fval);
...
// for blink node, insert the first key of `new_node` as fence key for `old`
...
static void node_get_whole_key(node *n, uint32_t idx, char *key, uint32_t *len) {
    assert(idx < n->keys);
    index_t *index = node_index(n);
...
// fence key, `nkey` is new_node fence key
void node_adjust_many(node *new_node, node *left, node *right, char *okey, uint32_t *olen,
    ...
    // get new_node node's index
    new_node->keys = lmk + rmk;
    index_t *n_idx = node_index(new_node);
    new_node->keys = 0;

    // WARN: we assume that `new_node` has enough space

    // step 2, move keys from `left` to `new_node`
    index_t *l_idx = node_index(left);
    ...
    // step 3, move keys from `right` to `new_node`
    index_t *r_idx = node_index(right);
...
// replace old key with new_node key, if old key and new_node key have the same key length, this is just an in-place update,
...
static int batch_write(batch *b, uint32_t op, const void *key1, uint32_t len1, const void *val) {
    ...
    // check if there is enough space
    if (unlikely(
            (char *) b->data + (b->off + sizeof(uint8_t) /* op */ + key_byte + len1 + value_bytes) > (char *) index))
        return -1;
